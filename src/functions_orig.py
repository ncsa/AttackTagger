...
# Time-stamp: <2014-09-29 18:44:11 phuong>
"""
...
class StateState():
    ...
    def fss_failed_password_suspicious(ep, sup, sap, e, su, sa):
        if (e == int(SE.login) and ep == int(SE.ALERT_FAILED_PASSWORD) and sup == int(SU.suspicious) and su == int(SU.suspicious) and sa == int(SA.prepare_attack)):
            return 1
        return 0
    ...
    def fss_sensitive_uri_malware_malicious(ep, sup, sap, e, su, sa):
        if e == int(SE.ALERT_MALWARE_HASH_REGISTRY_MATCH) and is_sensitive_uri(ep) and sup == int(SU.suspicious) and su == int(SU.malicious) and sa == int(SA.prepare_attack):
            return 1
        return 0
    # two sensitive URIs
    def fss_sensitive_uri_malware_malicious(ep, sup, sap, e, su, sa):
        if is_sensitive_uri(ep) and is_sensitive_uri(e) and sup == int(SU.suspicious) and su == int(SU.malicious) and sa == int(SA.prepare_attack):
            return 1
        return 0
